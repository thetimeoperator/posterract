// Builds src/styles/landing-cyan.css: every colour declaration of the landing
// stylesheets, scoped under .site[data-palette="cyan"], with each green hue
// turned cyan-blue in OKLCH (lightness and chroma kept, gamut-clamped). The
...
// ---- a small CSS walker ----------------------------------------------------
const SITE_VARS = /var\(--site-(?!edge|intro)/;
const HAS_TOKEN = new RegExp(TOKEN.source, "i");
const isColourDecl = (value) => HAS_TOKEN.test(value) || SITE_VARS.test(value);
...
function emit(nodes, indent = "") {
  ...
  for (const node of nodes) {
    ...
    const decls = declarations(node.body).filter((d) => isColourDecl(d.value));
