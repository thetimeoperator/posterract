...
/**
 * Archived Resonance coverage for the demo engine: the Points page renders,
...
test.describe("Archived Resonance points", () => {
  test("Points route stays hidden while the feature is archived", async ({ page }) => {
    await page.goto("/points");
    await expect(page).toHaveURL(/\/forge$/);
    await expect(page.getByText("Your content harness is online.")).toBeVisible();
  });

  test.skip("Points page renders rank ring and ladder", async ({ page }) => {
    await page.goto("/points");
  ...
  test.skip("publishing earns RP and shows on dashboard + ledger", async ({ page }) => {
  ...
  test("the post-login home is Create", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveURL(/\/forge/);
    await expect(page.getByText("Your content harness is online.")).toBeVisible();
  });
