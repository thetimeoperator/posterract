...
test("quotes price every kind, including edge durations, and reject bad params", () => {
  assert.deepEqual(
    ...
    { credits: 5, lineItems: [{ label: "Image 1k", credits: 5 }] },
  );
  assert.equal(
  ...
  assert.equal(shortVideo.credits, 16);
  assert.equal(shortVideo.lineItems[0].label, "Video 768p × 4s @ 4/s");
  assert.equal(
  ...
  assert.equal(quote("voice", FISH_VOICE_MODEL, { text: "a" }).credits, 2);
  assert.equal(
...
test("execute reserves credits, settles on success, and exposes the generation", async () => {
  ...
  try {
    ...
    assert.deepEqual(quoted.json(), {
      credits: 10,
      lineItems: [{ label: "Image 2k", credits: 10 }],
    });
    ...
    assert.equal(read.json().creditsSettled, 10);
    ...
    assert.equal(entries[1].delta, -10);
...
test("idempotency replay returns the first response without a second charge", async () => {
  ...
  try {
    ...
    assert.equal(Number(balance.rows[0].balance), 140);
  } finally {
    await app.close();
    await postgres.close();
  }
...
test("provider failures mark the generation failed and refund the balance", async () => {
  ...
  try {
    ...
    assert.equal(ledger.json().entries[0].delta, 10);
  } finally {
    await app.close();
    await postgres.close();
  }
...
test("insufficient balances get a 402 and the reservation is released", async () => {
  const { postgres, pool } = await database({ balance: 8 });
  const { app, ai } = await testApp(pool);
  const videoBody = {
    kind: "video",
  ...
  try {
    ...
    assert.equal(rejected.json().needed, 16);
    assert.equal(rejected.json().balance, 8);
    ...
    await ai.drain();
    const creditsState = await app.inject({ method: "GET", url: "/v1/credits" });
    assert.equal(creditsState.json().balance, 84);
  } finally {
    await app.close();
    await postgres.close();
  }
...
/**
 * The gate exists to protect the margin, not the balance, so it answers before
 * the ledger is touched: an editor subscriber is told to upgrade rather than
 * told they are out of credits, and no generation row is written either way.
 */
test("plans gate generation before any credit is reserved", async () => {
  const { postgres, pool } = await database();
  const { app } = await testApp(pool);
  try {
    await pool.query(
    ...
    const tooBig = await generate(app, "ai-plan-gate-0002", {
      kind: "video",
      model: MINIMAX_VIDEO_MODEL,
      params: { prompt: "p", resolution: "2k", durationSec: 4, aspectRatio: "9:16" },
      mode: "execute",
    });
