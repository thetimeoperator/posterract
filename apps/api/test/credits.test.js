      ...
      data: [
        {
          ...
          price: {
            id: environment.STRIPE_PRICE_CREATOR,
...
test("credit plan definitions expose the launch catalog", async () => {
  assert.equal(CREDIT_PLANS.creator.credits, 150);
  assert.equal(CREDIT_PLANS.studio.credits, 2_250);
  assert.equal(CREDIT_PLANS.agency.credits, 5_250);
  assert.equal(CREDIT_PLANS.creator.monthlyAmount, 2_000);
  assert.equal(CREDIT_PLANS.studio.monthlyAmount, 5_000);
  assert.equal(CREDIT_PLANS.agency.monthlyAmount, 10_000);
  ...
  try {
    ...
    assert.deepEqual(config.json().creditPlans.creator, {
    ...
    assert.equal(config.json().creditPlans.agency.credits, 5_250);
  } finally {
    await app.close();
...
test("paid credit-plan invoices set the plan and reset the balance each cycle", async () => {
  ...
  try {
    ...
    assert.equal(credits.rows[0].plan, "creator");
    ...
    const firstInvoice = await send({
      ...
      data: {
        object: paidInvoice({
          id: "in_credit_1",
          priceId: environment.STRIPE_PRICE_CREATOR,
    ...
    assert.equal(credits.rows[0].plan, "creator");
    assert.equal(Number(credits.rows[0].balance), 150);
    assert.equal(Number(credits.rows[0].allotment), 150);
    assert.equal(
    ...
    assert.equal(Number(grants.rows[0].delta), 150);
    assert.match(
      grants.rows[0].note,
      /Granted 150 creator credits for cycle \d{4}-\d{2}-\d{2} to \d{4}-\d{2}-\d{2}/,
    );
...
test("cancellation clears the plan but leaves the balance until cycle end", async () => {
  ...
  try {
    ...
    assert.equal(upgraded.rows[0].plan, "studio");
    assert.equal(Number(upgraded.rows[0].balance), 2_250);
    assert.equal(Number(upgraded.rows[0].allotment), 2_250);
  } finally {
    await app.close();
    await postgres.close();
  }
