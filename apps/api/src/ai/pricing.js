...
/**
 * Credit prices. Pure math: no I/O, no environment reads.
 *
 * **One credit is one cent of provider spend at our cost.** Pegging the unit
 * to money is what keeps a plan's margin readable: a 1,200-credit plan costs
 * at most $12.00 to serve, so the profit is knowable before anyone subscribes
 * rather than discovered at the end of the month.
 *
 * Every price rounds *up* to the next whole credit, so rounding always favours
 * us. Costs verified 2026-09-04 at list price; re-check them before a launch,
 * because every margin moves with them.
 *
 *   image 1k    Nano Banana 2      $0.045/image     ->  5
 *   image 2k    Nano Banana 2      $0.10/image      -> 10
 *   video 768p  MiniMax Hailuo     $0.04/second     ->  4/s
 *   video 2k    MiniMax Hailuo     $0.08/second     ->  8/s
 *   voice       Fish Audio s2-pro  $15.00/1M chars  ->  2/1k chars
 *   transcribe  Whisper            $0.006/minute    ->  1/minute
 *
 * Video is priced per second rather than per clip on purpose. A flat per-clip
 * rate is the one thing that can quietly destroy the margin: the same call at
 * 2k for 15 seconds costs six times a 768p six-second one, and a client that
 * asked for the expensive one would have paid for the cheap one.
 */
export const IMAGE_CREDITS = { "1k": 5, "2k": 10 };
export const VIDEO_CREDITS_PER_SECOND = { "768p": 4, "2k": 8 };
export const VOICE_CREDITS_PER_1K_CHARS = 2;
