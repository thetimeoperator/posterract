...
/**
 * The faces compositions draw with.
 *
 * `file` is the copy the build stages into the app's own assets (see
 * `scripts/vendor-fonts.mjs`); `url` is where it came from, and the fallback
 * for the web app, which serves no staged copy. The desktop renderer allows
 * no network at all, so without the staged file a preset silently falls back
 * to a serif — and an export made a year from now would depend on a CDN.
 */
