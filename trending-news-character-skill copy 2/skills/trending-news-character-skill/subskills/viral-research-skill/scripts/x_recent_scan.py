#!/usr/bin/env python3
"""Scan X recent search results for a configurable trending-news character project."""
...
def request_json(url: str, params: Dict[str, str], token: str) -> Dict:
    ...
    request = urllib.request.Request(
        f"{url}?{query}",
        headers={"Authorization": f"Bearer {token}", "User-Agent": "trending-news-character-x-scan"},
    )
    try:
...
def load_usage_state(path: pathlib.Path, daily_post_cap: int, max_api_calls: int, *, persist: bool = True) -> Dict[str, object]:
    today = local_today()
    if path.exists():
        with open(path, "r", encoding="utf-8") as handle:
            try:
...
def main() -> int:
    ...
    usage_before = load_usage_state(ledger_path, args.daily_post_cap, args.max_api_calls, persist=not args.dry_run)
